  ...
  const renderSection = (
    ...
    return items.map((item, index) => (
      <div className={styles['lawyer-career__item']} key={index}>
        <h4 className={styles['lawyer-career__item-title']}>
          {'lawyerCareerCategoryName' in item ? item.lawyerCareerCategoryName : item.categoryName}
        </h4>
        <ul className={styles['lawyer-career__list']}>
          <li key={index}>{'lawyerCareerContent' in item ? item.lawyerCareerContent : item.content}</li>
        </ul>
      </div>
    ))
  }
