...
export const useKnowledgeKeep = ({
  ...
  return useMutation({
    ...
    onSuccess: (data: KnowledgeKeepResponse, knowledgeId: number) => {
      // 무한 스크롤 쿼리 캐시 업데이트
      queryClient.setQueriesData({ queryKey: [QUERY_KEY.KNOWLEDGE_LIST, 'infinite'] }, (oldData: any) => {
      ...
      // 일반 리스트 쿼리 캐시 업데이트
      queryClient.setQueriesData({ queryKey: [QUERY_KEY.KNOWLEDGE_LIST] }, (oldData: any) => {
